...
export function AcceptanceBonusPage({ onCreateBonusBon }: { onCreateBonusBon: (customerCode: string) => void }) {
  ...
  const remainder = selectedCustomer.accumulatedPaidOmzet % selectedCustomer.bonusThreshold;
  ...
  return (
    <section className="acceptance-page">
      ...
      <div className="acceptance-info-banner"><Info size={23} /><div><strong>Aturan bonus client</strong><span>Setiap bonus memakai satu threshold omzet Lunas. Produk bonus memiliki harga Rp0 dan tidak memengaruhi laba HL.</span></div></div>

      <div className="acceptance-stat-grid acceptance-stat-grid--three">
        ...
        <Stat label="Bonus Bon Tercatat" value={`${acceptanceBons.filter((bon) => bon.isBonus).length} Bon`} helper="Dilaporkan terpisah" icon={<ReceiptText size={23} />} />
      </div>
      ...
      <section className="acceptance-card">
        ...
        {bonusBons.length === 0 ? <div className="acceptance-inline-empty"><FileText size={30} /><span>Belum ada Bonus Bon untuk pelanggan ini.</span></div> : <div className="bonus-bon-history">{bonusBons.map((bon) => <article key={bon.number}><span><strong>{bon.number}</strong><small>{toDisplayDate(bon.date)} · {bon.lines.reduce((sum, line) => sum + line.quantity, 0)} unit bonus</small></span><strong>{formatCurrency(0)}</strong></article>)}</div>}
      </section>
    </section>
  );
}
  ...
  return (
    <section className="acceptance-page">
      <header className="acceptance-page-header">
        ...
        <div><span className="eyebrow">Cash basis</span><h2>Laporan</h2><p>Omzet dan laba hanya berasal dari Bon Lunas. Bonus Bon dilaporkan terpisah.</p></div>
        <button className="button button--primary" type="button" onClick={downloadPdf}><Download size={20} />Unduh PDF</button>
      </header>

      {downloaded && <div className="success-banner"><CheckCircle2 size={23} /><div><strong>PDF sedang disiapkan</strong><span>Pada fase integrasi, file akan diambil dari PDF API.</span></div></div>}

      <section className="acceptance-filter-card report-acceptance-filter">
        <div><span className="eyebrow">Filter rekap</span><h3>Pilih Periode dan Cakupan</h3></div>
        ...
        <label className="field"><span>Pelanggan</span><select value={customerCode} onChange={(event) => setCustomerCode(event.target.value)}><option value="all">Semua pelanggan</option>{customerProfiles.filter((customer) => customer.active).map((customer) => <option value={customer.code} key={customer.code}>{customer.name}</option>)}</select></label>
        <label className="field"><span>Cakupan produk</span><select value={scope} onChange={(event) => setScope(event.target.value as "overall" | "LM" | "BR")}><option value="overall">Keseluruhan</option><option value="LM">LM</option><option value="BR">BR</option></select></label>
      </section>
      ...
      <div className="acceptance-stat-grid">
        ...
        <Stat label="Laba HL Lunas" value={formatCurrency(totalProfit)} helper="Ongkir dan bonus tidak memengaruhi" icon={<BarChart3 size={23} />} />
      ...
      <div className="acceptance-detail-layout">
        ...
        <aside className="acceptance-card"><div className="acceptance-card-heading"><div><span className="eyebrow">Bonus log</span><h3>Bonus Bon</h3></div><Gift size={24} /></div><div className="bonus-log-summary"><strong>{bonusRows.length} Bon</strong><span>{bonusRows.reduce((sum, bon) => sum + bon.lines.reduce((lineSum, line) => lineSum + line.quantity, 0), 0)} unit bonus</span><small>Omzet Rp0 · Laba Rp0</small></div></aside>
      </div>

      <section className="acceptance-card">
        <div className="acceptance-card-heading"><div><span className="eyebrow">Daftar transaksi</span><h3>Hasil Rekap {monthKey}</h3></div><FileText size={24} /></div>
        {rows.length === 0 ? <div className="acceptance-inline-empty"><FileText size={30} /><span>Tidak ada data untuk filter ini.</span></div> : <div className="acceptance-report-wrap"><table className="acceptance-report-table"><thead><tr><th>Tanggal</th><th>Nomor Bon</th><th>Pelanggan</th><th>Status</th><th>Omzet</th><th>Ongkir</th><th>Total</th><th>Laba</th></tr></thead><tbody>{rows.map((bon) => { const totals = calculateBon(bon); return <tr key={bon.number}><td>{toDisplayDate(bon.date)}</td><td><strong>{bon.number}</strong></td><td>{totals.customer.name}</td><td>{bon.status}</td><td>{formatCurrency(totals.omzet)}</td><td>{formatCurrency(bon.shipping)}</td><td>{formatCurrency(totals.amountOwed)}</td><td>{formatCurrency(totals.profit)}</td></tr>; })}</tbody></table><div className="acceptance-report-mobile">{rows.map((bon) => { const totals = calculateBon(bon); return <article key={bon.number}><header><span><strong>{bon.number}</strong><small>{totals.customer.name}</small></span><strong>{bon.status}</strong></header><dl><div><dt>Tanggal</dt><dd>{toDisplayDate(bon.date)}</dd></div><div><dt>Omzet</dt><dd>{formatCurrency(totals.omzet)}</dd></div><div><dt>Ongkir</dt><dd>{formatCurrency(bon.shipping)}</dd></div><div><dt>Total</dt><dd>{formatCurrency(totals.amountOwed)}</dd></div><div><dt>Laba</dt><dd>{formatCurrency(totals.profit)}</dd></div></dl></article>; })}</div></div>}
      </section>
    </section>
  );
}
