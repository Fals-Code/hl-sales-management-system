...
export function AcceptanceCustomersPage({
  ...
  const saveCustomer = (nextCustomer: CustomerProfile) => {
    setCustomers((current) => {
      const exists = current.some((customer) => customer.code === nextCustomer.code);
      return exists ? current.map((customer) => customer.code === nextCustomer.code ? nextCustomer : customer) : [...current, nextCustomer];
    });
    setEditor(null);
  };
      ...
      {filteredCustomers.length === 0 ? (
        ...
        <div className="acceptance-customer-grid">
          {filteredCustomers.map((customer) => {
            ...
            return (
              <button className="acceptance-customer-card" type="button" key={customer.code} onClick={() => setSelectedCode(customer.code)}>
                <span className="customer-avatar">{customer.name.slice(0, 2).toUpperCase()}</span>
                <span><span className="eyebrow">{customer.code}</span><strong>{customer.name}</strong><small>{customer.address} · {customer.phone}</small><span className="customer-card-facts"><span>Piutang <strong>{formatCurrency(receivable)}</strong></span><span>Bonus <strong>{bonusesAvailable(customer)} unit</strong></span></span></span>
...
function AcceptanceCustomerDetail({
  customer,
  onBack,
  onEdit,
  onDelete,
  ...
  const monthlyBons = acceptanceBons.filter((bon) => bon.customerCode === customer.code && bon.date.startsWith(monthKey));
  ...
  return (
    <section className="acceptance-page">
      ...
      <header className="acceptance-customer-hero">
        ...
        <div className="customer-hero-actions"><button className="button button--secondary" type="button" onClick={onEdit}><Pencil size={19} />Edit</button><button className="button button--secondary" type="button" onClick={onSettlement}><HandCoins size={19} />Pelunasan</button><button className="button button--primary" type="button" onClick={onCreateBon}><Plus size={19} />Buat Bon</button><button className="button button--danger" type="button" onClick={onDelete}><Trash2 size={19} />Nonaktifkan</button></div>
      </header>

      <section className="acceptance-filter-card">
        <div><span className="eyebrow">Periode transaksi</span><h3>Pilih Bulan dan Tahun</h3></div>
        ...
        <button className="button button--secondary" type="button"><Download size={19} />Unduh PDF</button>
      </section>
      ...
      <div className="acceptance-detail-layout">
        <section className="acceptance-card">
          ...
          {monthlyBons.length === 0 ? <div className="acceptance-inline-empty"><ReceiptText size={30} /><span>Belum ada transaksi pada periode ini.</span></div> : <div className="acceptance-bon-list">{monthlyBons.map((bon) => { const totals = calculateBon(bon); return <button className="acceptance-bon-row" type="button" key={bon.number} onClick={() => onViewBon(bon.number)}><span><strong>{bon.number}</strong><small>{toDisplayDate(bon.date)} · {bon.status}</small></span><strong>{formatCurrency(totals.amountOwed)}</strong><ChevronRight size={20} /></button>; })}</div>}
        </section>
