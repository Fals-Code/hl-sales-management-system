...
function EditForm({
  ...
  const draft = useMemo<StoredBon>(
    () => ({
      ...bon,
      date,
      customerCode,
      description,
      shipping: roundToHundred(Math.max(0, shipping)),
      lines
    }),
    [bon, date, customerCode, description, shipping, lines]
  );
  ...
  const save = async () => {
    ...
    try {
      if (useApi) {
        ...
        if (!bon.backendId || !customer?.backendId) {
          throw new Error("Data Bon atau pelanggan belum memiliki ID backend.");
        }
        const items = lines.map((line) => {
          const product = products.find((entry) => entry.id === line.productId);
          if (!product?.backendId) {
            throw new Error("Satu atau lebih produk belum memiliki ID backend.");
          }
          return {
  ...
  return (
    <div className="dialog-backdrop acceptance-dialog-backdrop" role="presentation" onMouseDown={onClose}>
      <section
        className="acceptance-master-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header>
          <div>
            <span className="eyebrow">Bon Piutang</span>
            <h2>Edit {bon.number}</h2>
            <p>Perubahan menghitung ulang omzet, laba, ongkir, dan snapshot transaksi.</p>
          </div>
          <button className="icon-button" type="button" disabled={saving} onClick={onClose}>
            <X size={22} />
          </button>
        </header>
        ...
        <div className="acceptance-master-body">
          <div className="acceptance-form-grid">
            <label className="field">
              ...
              <div className="input-icon-shell">
                ...
                <input type="date" value={date} disabled={saving} onChange={(event) => setDate(event.target.value)} />
              </div>
            </label>
            <label className="field">
              <span>Nomor Bon</span>
              <input value={bon.number} disabled />
              <small>Nomor Bon tidak dapat diubah saat edit.</small>
            </label>
            <label className="field field--wide">
              <span>Pelanggan *</span>
              <select value={customerCode} disabled={saving} onChange={(event) => setCustomerCode(event.target.value)}>
                {customers.filter((entry) => entry.active).map((entry) => (
            ...
            {lines.map((line, index) => (
              <article className="acceptance-line-card" key={`${line.productId}-${index}`}>
                <label className="field">
                  ...
                  >
                    {products.filter((entry) => entry.active).map((entry) => (
                      <option value={entry.id} key={entry.id}>{entry.name} · {entry.type}</option>
                    ))}
                  </select>
                </label>
                <label className="field">
          ...
          {totals.negativeProfit && (
            <section className="negative-profit-card">
              <div>
                <AlertTriangle size={23} />
                <span>
                  <strong>Perubahan menghasilkan laba negatif</strong>
                  <small>PIN Owner dan alasan wajib diisi.</small>
                </span>
              </div>
              <div className="acceptance-form-grid">
                <label className="field">
                ...
                <label className="field field--wide">
                  <span>Alasan *</span>
                  <textarea rows={3} value={reason} disabled={saving} onChange={(event) => setReason(event.target.value)} />
                </label>
              </div>
            </section>
          )}
          ...
          <aside className="acceptance-live-summary">
            ...
            <Summary label="Omzet" value={totals.omzet} />
            <Summary label="Ongkir" value={draft.shipping} />
            <Summary label="Laba HL" value={totals.profit} />
            <div className="acceptance-total">
              <span>Total Piutang</span>
              <strong>{formatCurrency(totals.amountOwed)}</strong>
            </div>
          </aside>
        ...
        <footer>
          <button className="button button--secondary" type="button" disabled={saving} onClick={onClose}>Batal</button>
          <button className="button button--primary" type="button" disabled={!valid || saving} onClick={() => { void save(); }}>
            <Save size={20} />
