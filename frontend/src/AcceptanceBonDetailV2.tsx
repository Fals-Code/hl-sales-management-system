...
function SensitiveDialog({ action, bonNumber, ownerPin, setOwnerPin, reason, setReason, processing, onClose, onConfirm }: { action: "delete" | "void" | null; bonNumber: string; ownerPin: string; setOwnerPin: (value: string) => void; reason: string; setReason: (value: string) => void; processing: boolean; onClose: () => void; onConfirm: () => void }) { if (!action) return null; const voidAction = action === "void"; const valid = !voidAction || (ownerPin.length >= 4 && reason.trim().length >= 8); return <div className="dialog-backdrop acceptance-dialog-backdrop" role="presentation" onMouseDown={onClose}><section className="acceptance-master-dialog acceptance-master-dialog--small" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><header><div><span className="eyebrow">{voidAction ? "Otorisasi Owner" : "Soft-delete"}</span><h2>{voidAction ? "Void Bon Lunas" : "Nonaktifkan Bon Piutang"}</h2><p>{bonNumber} tetap tersimpan dalam riwayat audit.</p></div><button className="icon-button" type="button" disabled={processing} onClick={onClose}><X size={22} /></button></header><div className="acceptance-master-body">{voidAction ? <><label className="field"><span>PIN Owner *</span><input type="password" inputMode="numeric" maxLength={32} disabled={processing} value={ownerPin} onChange={(event) => setOwnerPin(event.target.value.replace(/\D/g, "").slice(0, 32))} /></label><label className="field"><span>Alasan Void *</span><textarea rows={3} disabled={processing} value={reason} onChange={(event) => setReason(event.target.value)} /></label></> : <div className="acceptance-warning"><AlertTriangle size={20} /><span>Bon disembunyikan dari transaksi aktif, tetapi data historis tidak dihapus.</span></div>}</div><footer><button className="button button--secondary" type="button" disabled={processing} onClick={onClose}>Batal</button><button className="button button--danger" type="button" disabled={!valid || processing} onClick={onConfirm}>{processing ? "Memproses..." : voidAction ? "Void Bon" : "Nonaktifkan Bon"}</button></footer></section></div>; }
