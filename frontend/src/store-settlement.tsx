...
import { calculateBon, currentIsoDate, toDisplayDate, type AcceptanceBon } from "./acceptance-data";
...
export function StoreSettlement({ prefillCustomerCode, onViewBon }: { prefillCustomerCode?: string | null; onViewBon: (bonNumber: string) => void }) {
  const { customers, bons, updateBonStatus } = useAppStore();
  ...
  return <section className="acceptance-page">
    ...
    <section className="acceptance-filter-card settlement-filter-card"><label className="field"><span>Pelanggan</span><div className="input-icon-shell"><UserRound size={21} /><select value={customerCode} onChange={(event) => { setCustomerCode(event.target.value); setSelectedNumbers([]); setNotice(null); }}>{activeCustomers.map((entry) => <option value={entry.code} key={entry.code}>{entry.name}</option>)}</select></div></label><label className="field"><span>Periode Bon</span><input type="month" value={period} onChange={(event) => { setPeriod(event.target.value); setSelectedNumbers([]); setNotice(null); }} /></label><label className="field"><span>Tanggal Pelunasan *</span><div className="input-icon-shell"><CalendarDays size={21} /><input type="date" value={paymentDate} onChange={(event) => setPaymentDate(event.target.value)} /></div></label></section>
